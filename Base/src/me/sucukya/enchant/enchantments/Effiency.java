//Programmed by Sucukya
//Don't pretend this code is yours.

package me.sucukya.enchant.enchantments;


import de.tr7zw.nbtapi.NBTItem;
import me.sucukya.utility.Data;
import org.bukkit.Material;
import org.bukkit.enchantments.Enchantment;
import org.bukkit.inventory.ItemFlag;
import org.bukkit.inventory.meta.ItemMeta;

public class Effiency {

    private static String type = "pickaxe";
    private static String type2 = "axe";
    private static String type3 = "shovel";

    

    public static void doEnchant(org.bukkit.entity.Player p, org.bukkit.inventory.ItemStack stack, java.lang.Integer level){
        NBTItem nbti = new NBTItem(stack);
        
                if(stack.getType().equals(Material.BOOK)) {
                    nbti.setInteger("e10", level);
                    nbti.setInteger("totalenchs", nbti.getInteger("totalenchs") + 1);
                    nbti.setString("type","book");
                    p.sendMessage(Data.prefix + "§a Successfully applied §2Effiency " + level + "§a!");
                    stack = nbti.getItem();
                    ItemMeta meta = stack.getItemMeta();
                    meta.setUnbreakable(true);
                    stack.setItemMeta(meta);
                    me.sucukya.utility.loreRebuilder.rebuild(stack, p, p.getInventory());
                    
                } else {
                    String type1 = nbti.getString("type");
                    if (type1.equalsIgnoreCase(type) || type1.equalsIgnoreCase(type2) || type1.equalsIgnoreCase(type3)) {
                        nbti.setInteger("e10", level);
                        nbti.setInteger("totalenchs", nbti.getInteger("totalenchs") + 1);
                        stack = nbti.getItem();
                        p.sendMessage(Data.prefix + "§a Successfully applied §2Effiency " + level + "§a!");
                        stack.addUnsafeEnchantment(Enchantment.DIG_SPEED, level);
                        ItemMeta meta = stack.getItemMeta();
                        meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES);
                        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
                        meta.addItemFlags(ItemFlag.HIDE_UNBREAKABLE);
                        meta.setUnbreakable(true);
                        stack.setItemMeta(meta);
                        me.sucukya.utility.loreRebuilder.rebuild(stack, p, p.getInventory());

                    } else {
                        p.sendMessage(Data.prefix + "§cThis enchantment can't be put on your selected item!");
                    }
                }

    }

}
